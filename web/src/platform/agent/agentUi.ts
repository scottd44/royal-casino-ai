/* ============================================================
   ...
   The UI (bottom "AI Lab" drawer) lives in modules/casino/lab and reads
   from the API this module exposes via `royalAgent` / `useAgentStore`.

   Ported line-for-line from js/agent/agent-ui.js (plan §3). Same functions,
   same order, same behavior, same `localStorage` key. The only substantive
   changes are the three called out in plan §0.3:
     1. Casino.fmt/money/pick/randInt/getBalance/payout/toast -> real ES
        imports (@/platform/money/format, @/platform/money/walletStore,
        @/platform/ui/toast) instead of window.Casino.
     2. OllamaAgent's endpoint is passed through its own constructor option
        instead of legacyBridge.ts's classic-script prototype patch.
     3. window.RoyalAgent -> window.__royalAgentDebug, DEV-only.
   Everything else — GAMBLER_PERSONA, IMPROV, settings, all 25 adapters,
   $/sleep/pace/settle/waitFor, setStatus, LOG_KEY, the hashchange listener,
   the public API shape — is unchanged.
   ============================================================ */

import { OllamaAgent, OllamaAgentError } from './harness'
...
/* ------------------------------------------------------------
   Phase-3 game globals (plan §8 — out of scope here). Ten of these 25
   adapters drive a game that hasn't been ported to React yet; each still
   attaches its API to `window` exactly like every other classic script in
   js/games/. Typed here ONLY to the surface this file's adapters actually
   touch — Phase 3 owns the real interface the day each game is ported.
   ------------------------------------------------------------ */
type Card = { rank: string; suit: { s: string } }

type RouletteAPIType = {
  clearBets: () => void
...
type VideoPokerEval = { key: string; label: string } | null
type VideoPokerGameType = {
  evaluate: (cards: Card[]) => VideoPokerEval
}

type HoldemMove = { type: string; amount?: number }
type HoldemState = {
  hole: string[]
  board: string[]
  pot: number
...
type CoinflipAPIType = {
  setCoins: (n: number) => void
  call: (side: number) => void
  inRound: () => boolean
  streak: () => number
  cashOut: () => void
}
...
declare global {
  interface Window {
    RouletteAPI?: RouletteAPIType
    VideoPokerGame?: VideoPokerGameType
    HoldemAPI?: HoldemAPIType
    ThreeCardGame?: ThreeCardGameType
    BattleshipAPI?: BattleshipAPIType
    MolesAPI?: MolesAPIType
    CoinflipAPI?: CoinflipAPIType
    RPSAPI?: RPSAPIType
    SnakesAPI?: SnakesAPIType
    KenoAPI?: KenoAPIType
    /** DEV-only console/debugging handle — plan §0.3.3, parity with
     *  platform/motion/watchdog.ts's window.__royalMotion. */
    __royalAgentDebug?: RoyalAgent
  }
}

/** Endpoint fallback matches legacyBridge.ts's OLLAMA_ENDPOINT exactly (plan
 * §0.3.2) — legacyBridge.ts is retired by this file, so this is now the one
 * place that computes it. */
const OLLAMA_ENDPOINT = import.meta.env.VITE_OLLAMA_ENDPOINT || '/ollama/api/chat'

export function createRoyalAgent(): RoyalAgent {
  ...
  let onUpdate: () => void = () => {}
  // Report payload is data, not one of types.ts's Phase-0/1 surfaces — kept
  // loose here and narrowed only where LabReportModal (a different owner)
  // consumes it, same reasoning as LogEvent below.
  let onReport: (report: Record<string, unknown>) => void = () => {}
  ...
  /** Matches types.ts's AgentLogEvent exactly (that type is permissive on
   * purpose — see its doc comment): session events carry
   * model/agentic/trigger, break events carry phase/plannedMs/actualMs,
   * desperate-financing pushes `kind: 'loan'` with label/amount/reason.
   * Declared locally rather than imported so this file doesn't take on a
   * dependency for what's a structural, not nominal, match. */
  type LogEvent = { kind: string; t?: number } & Record<string, unknown>

  let session: SessionState | null = null // aggregate stats for the current run
  let rounds: AgentRound[] = [] // [{ n, wager, delta, balance, game }]
  let log: LogEvent[] = loadLog() // unified, PERSISTED event feed (moves, results, breaks, session)
  let latencies: number[] = []
  let decisions = 0, fallbacks = 0, retried = 0, totalWagered = 0
  let lastBet = 0 // stake of the round currently in progress
  let lastStopReason = ''
  let gameNet: Record<string, number> = {} // net profit per game this session (agentic context)
  let gameWL: Record<string, { w: number; l: number }> = {} // per-game win/loss record
  let tilt = 0 // 0-100 emotional state — rises on losses, cools on wins
  let recentGames: string[] = [] // last few games played (agentic variety)
  let borrowed = 0 // total "desperate financing" injected after going broke
  let desperateMeasures: Array<{
    n: number
    action: string
    label: string
    amount: number
  ...
  function recordRound(delta: number, wager: number, game: string) {
  ...
  function sessionContext() {
    ...
    return {
      ...
      improv: pick(IMPROV) + ' — ' + pick(IMPROV), // fresh style nudge every decision
    }
  }
  ...
  /* ---------------- tiny DOM helpers ----------------
     $ keeps the exact vanilla implementation and call-site shape (plan §3 —
     no per-call-site querySelector<T> generics across ~200 sites, too much
     transcription risk for zero behavioral gain). The one concession strict
     TypeScript requires is a single cast on $ itself, made once, to a type
     wide enough to cover every field the call sites in this file actually
     read off it (grepped: click/dataset/disabled/style/textContent/value —
     exactly HTMLInputElement's own surface plus what it inherits from
     HTMLElement/Node). Like the original, this does NOT null-check: a
     missing selector still throws at the point of use, exactly like
     `document.querySelector(sel).click()` did in agent-ui.js. */
  ...
  async function waitFor(pred: () => boolean, timeoutMs = 8000, stepMs = 60): Promise<boolean> {
  ...
  function clampBet(requested: unknown) {
    const bal = wallet.getBalance()
  ...
  function applyBet(requested: unknown) {
  ...
  function pushMove(
    ...
    const ev: LogEvent = {
      ...
      balance: wallet.getBalance(), // running bankroll at decision time
    }
    if (args && Array.isArray(args.bets)) {
      ev.bets = (args.bets as Array<{ spot: unknown; number: unknown; amount: unknown }>).map((b) => ({
        spot: b.spot, number: b.number, amount: b.amount,
      }))
    }
    logEvent(ev)
  }
  ...
  function annotateLastMove(patch: Record<string, unknown>) {
  ...
  function pushResult(delta: number, game: string) {
    logEvent({ kind: 'result', round: rounds.length, game, delta, balance: wallet.getBalance() })
  }
  ...
  type DecideResult = { action: string; args: Record<string, unknown> }
   ...
   */
  async function decideOrDefault(params: DecideParams, fallback: () => DecideResult): Promise<DecideResult> {
    ...
    try {
      ...
      console.warn('[agent] using fallback:', err instanceof Error ? err.message : String(err))
      return fb
    }
  }
  ...
  const adapters: Record<string, GameAdapter> = {
    slots: {
      ...
      async play(ctx) {
        ...
        ctx.setStatus(`Spinning for ${fmt(lastBet)}…`)
        $('#spinBtn').click()
        await sleep(120)
    ...
    gems: {
      ...
      async play(ctx) {
        ...
        applyBet(dec.args.bet)
        ctx.setStatus(`Spinning gems for ${fmt(lastBet)}…`)
        $('#gemSpinBtn').click()
        await sleep(120)
    ...
    blackjack: {
      ...
      async play(ctx) {
        ...
        const BUTTON: Record<string, string> = { hit: '#hitBtn', stand: '#standBtn', double: '#doubleBtn' }
        const cardRank = (el: Element): string => {
        ...
        const betDec = await decideOrDefault(
          {
            ...
            extraArgs: { bet: { type: 'integer', description: `dollars to wager, 1..${wallet.getBalance()}` } },
          },
    ...
    videopoker: {
      ...
      async play(ctx) {
        ...
        const fallbackHolds = (cards: Card[]) => {
          const made = window.VideoPokerGame!.evaluate(cards)
        ...
        const holds = [...new Set(((Array.isArray(dec.args.hold) ? dec.args.hold : []) as unknown[])
          .map(Number).filter((i) => Number.isInteger(i) && i >= 0 && i <= 4))]
        annotateLastMove({ detail: { bet: lastBet, hold: holds.join(',') || 'none' } })
        holds.forEach((i) => {
          const slot = document.querySelector<HTMLElement>(`#vpCards .vp-slot[data-idx="${i}"]`)
    ...
    roulette: {
      ...
      async play(ctx) {
        ...
        const SPOTS: Record<string, string> = {
          red: 'red', black: 'black', even: 'even', odd: 'odd', low: 'low', high: 'high',
        ...
        const parsed: { key: string; amt: number; label: string }[] = []
        const rawBets = (Array.isArray(dec.args.bets) ? dec.args.bets : []) as Array<Record<string, unknown>>
        for (const b of rawBets) {
          if (!b) continue
          let key: string | undefined, label: string
          if (b.spot === 'straight') {
            const num = Number(b.number)
            if (!Number.isInteger(num) || num < 0 || num > 36) continue
            key = 'n:' + num; label = `straight ${num}`
          } else {
            key = SPOTS[b.spot as string]; if (!key) continue; label = b.spot as string
          }
        ...
        if (sum > bal) {
          ...
          while (sum > bal && parsed.length > 1) { const popped = parsed.pop(); sum -= popped ? popped.amt : 0 }
        ...
        ctx.setStatus(`Placing ${parsed.length} bet(s) totalling ${fmt(sum)}…`)
        api.spin()
        await sleep(200)
    ...
    dice: {
      ...
      async play(ctx) {
        ...
        const dec = await decideOrDefault(
          ...
          () => ({ action: Math.random() < 0.5 ? 'under' : 'over', args: { target: randInt(15, 85), bet: autoBetValue(settings.aggression) } }),
        )
        ...
        applyBet(dec.args.bet)
        ctx.setStatus(`Roll ${dec.action} ${target} for ${fmt(lastBet)}…`)
        $('#rollBtn').click()
        await sleep(150)
    ...
    mines: {
      ...
      async play(ctx) {
        ...
        const setup = await decideOrDefault(
          ...
          () => ({ action: 'start', args: { mines: pick([1, 2, 3, 3, 4, 5, 6, 8, 10]), bet: autoBetValue(settings.aggression) } }),
        )
        const mineCount = nearest(Number(setup.args.mines) || 3, MINE_OPTIONS)
        $('#mineSelect').value = String(mineCount)
        applyBet(setup.args.bet)
        ctx.setStatus(`Starting with ${mineCount} mines, bet ${fmt(lastBet)}…`)
        $('#startBtn').click()
        await sleep(300)
        ...
        while (!ctx.shouldStop() && $('#startBtn').disabled && guard++ < 25) {
          ...
          const available = tiles.map((_t, i) => i).filter((i) => !tiles[i].classList.contains('revealed'))
    ...
    crash: {
      ...
      async play(ctx) {
        ...
        ctx.setStatus(`Betting ${fmt(bet)}, auto cash-out at ${target.toFixed(2)}×…`)
        $('#placeBetBtn').click()
        await sleep(150)
    ...
    limbo: {
      ...
      async play(ctx) {
        ...
        const dec = await decideOrDefault(
          ...
          () => ({ action: 'play', args: { bet: autoBetValue(settings.aggression), target: pick([1.5, 2, 2, 3, 5, 10, 25]) } }),
        )
        ...
        ctx.setStatus(`Firing at ${target.toFixed(2)}× for ${fmt(bet)}…`)
        $('#limboBtn').click()
        await sleep(150)
    ...
    hilo: {
      ...
      async play(ctx) {
        const rankOf = (lbl: string) => ({ A: 1, J: 11, Q: 12, K: 13 } as Record<string, number>)[lbl] ?? Number(lbl)
        const readCard = () => {
    ...
    tower: {
      ...
      async play(ctx) {
        const TDIFF: Record<string, { tiles: number; safe: number }> = {
        ...
        const setup = await decideOrDefault(
          {
            ...
            extraArgs: {
              ...
              bet: { type: 'integer', description: `dollars to wager, 1..${wallet.getBalance()}` },
            },
            extraRequired: ['difficulty', 'bet'],
          },
          () => ({ action: 'start', args: { difficulty: pick(DIFFS), bet: autoBetValue(settings.aggression) } }),
        )
        const diff = DIFFS.includes(setup.args.difficulty as string) ? (setup.args.difficulty as string) : 'easy'
        ...
        while ($('#towerStart').disabled && !ctx.shouldStop() && guard++ < 12) {
          ...
          if (!activeRow) break
          const tiles = [...activeRow.querySelectorAll<HTMLElement>('.tile')]
          const cols = tiles.map((_t, i) => i).filter((i) => !tiles[i].classList.contains('revealed'))
    ...
    wheel: {
      ...
      async play(ctx) {
        ...
        const setup = await decideOrDefault(
          ...
          () => ({ action: 'spin', args: { bet: autoBetValue(settings.aggression), risk: pick(['low', 'medium', 'medium', 'high', 'risky']) } }),
        )
        const risk = ['low', 'medium', 'high', 'risky'].includes(setup.args.risk as string) ? (setup.args.risk as string) : 'medium'
        const riskBtn = document.querySelector<HTMLElement>(`[data-risk="${risk}"]`)
        if (riskBtn) riskBtn.click()
        const bet = applyBet(setup.args.bet)
        ctx.setStatus(`Spinning ${risk} wheel for ${fmt(bet)}…`)
        $('#wheelBtn').click()
        await sleep(200)
    ...
    chicken: {
      ...
      async play(ctx) {
        const CSURV: Record<string, number> = { easy: 0.91, medium: 0.83, hard: 0.72, daredevil: 0.55 }
    ...
    holdem: {
      ...
      async play(ctx) {
        ...
        while (!ctx.shouldStop() && guard++ < 60) {
          ...
          const type = legal.includes(dec.action) ? dec.action : (legal.includes('check') ? 'check' : 'call')
          const move: HoldemMove = { type }
          if (type === 'raise') {
          ...
          API.act(move)
          ctx.setStatus(`Poker: ${type}${move.amount ? ' to ' + fmt(move.amount) : ''}`)
          await sleep(400)
        }
    ...
    baccarat: {
      ...
      async play(ctx) {
        ...
        document.querySelector<HTMLElement>(`[data-side="${sideKey}"]`)?.click()
        applyBet(dec.args.bet)
        ctx.setStatus(`Baccarat: ${sideKey} for ${fmt(lastBet)}…`)
        $('#bacDeal').click()
        await sleep(200)
        await waitFor(() => $('#bacDeal') && !$('#bacDeal').disabled, 7000)
        ctx.setStatus($('#bacResult') ? $('#bacResult').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    threecard: {
      ...
      async play(ctx) {
        ...
        const cards: Card[] = [...document.querySelectorAll('#tcpPlayer .card .rank-top')].map((e) => {
        ...
        const dec = await decideOrDefault(
          ...
          () => {
            ...
            let play: boolean
            try {
              const T = window.ThreeCardGame!._t
              ...
              play = pairMatch || v[0] > 12 || (v[0] === 12 && (v[1] > 6 || (v[1] === 6 && v[2] >= 4)))
            }
    ...
    casinowar: {
      ...
      async play(ctx) {
        ...
        ctx.setStatus($('#warResult') ? $('#warResult').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    reddog: {
      ...
      async play(ctx) {
        ...
        ctx.setStatus($('#rdResult') ? $('#rdResult').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    battleship: {
      ...
      async play(ctx) {
        ...
        applyBet(setup.args.bet)
        $('#bsDeal').click()
        ctx.setStatus(`Battleship: firing for ${fmt(lastBet)}…`)
        await sleep(300)
        ...
        ctx.setStatus($('#bsStatus') ? $('#bsStatus').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    moles: {
      ...
      async play(ctx) {
        const API = window.MolesAPI!, bal = wallet.getBalance()
        const dec = await decideOrDefault(
          {
          ...
          () => ({ action: 'start', args: { bet: autoBetValue(settings.aggression), moles: randInt(2, 5) } }),
        )
        applyBet(dec.args.bet)
        ...
        await sleep(300)
        const target = randInt(1, Math.min(3, m))
        let g = 0
        ...
        ctx.setStatus($('#molMsg') ? $('#molMsg').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    coinflip: {
      ...
      async play(ctx) {
        const API = window.CoinflipAPI!, bal = wallet.getBalance()
        const dec = await decideOrDefault(
          {
        ...
        ctx.setStatus($('#cfMsg') ? $('#cfMsg').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    rps: {
      ...
      async play(ctx) {
        const API = window.RPSAPI!, bal = wallet.getBalance()
        const dec = await decideOrDefault(
          {
          ...
          () => ({ action: ['rock', 'paper', 'scissors'][randInt(0, 2)], args: { bet: autoBetValue(settings.aggression) } }),
        )
        applyBet(dec.args.bet)
        const moveMap: Record<string, number> = { rock: 0, paper: 1, scissors: 2 }
        const target = randInt(1, 4)
        API.throw(moveMap[dec.action] != null ? moveMap[dec.action] : randInt(0, 2)); await sleep(520)
        let g = 0
        while (API.inRound() && API.streak() < target && g++ < 12 && !ctx.shouldStop()) { API.throw(randInt(0, 2)); await sleep(520) }
        if (API.inRound() && API.streak() > 0) API.cashOut()
        ctx.setStatus($('#rpsMsg') ? $('#rpsMsg').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    snakes: {
      ...
      async play(ctx) {
        const API = window.SnakesAPI!, bal = wallet.getBalance()
        const dec = await decideOrDefault(
          {
          ...
          () => ({ action: 'roll', args: { bet: autoBetValue(settings.aggression), snakes: pick([1, 3, 3, 5, 7]) } }),
        )
        applyBet(dec.args.bet)
        ...
        ctx.setStatus($('#snMsg') ? $('#snMsg').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    keno: {
      ...
      async play(ctx) {
        const API = window.KenoAPI!, bal = wallet.getBalance()
        const dec = await decideOrDefault(
          {
          ...
          () => ({ action: 'play', args: { bet: autoBetValue(settings.aggression), risk: pick(['low', 'classic', 'classic', 'medium', 'high']) } }),
        )
        applyBet(dec.args.bet)
        API.setRisk(['low', 'classic', 'medium', 'high'].includes(dec.args.risk as string) ? (dec.args.risk as string) : 'classic')
        if (API.pickCount() === 0) API.quickPick()
        ctx.setStatus('Keno: drawing…')
        API.play()
        await sleep(400)
        ctx.setStatus($('#knMsg') ? $('#knMsg').textContent || 'Done.' : 'Done.')
      },
    },
    ...
    plinko: {
      ...
      async play(ctx) {
        ...
        const dec = await decideOrDefault(
          ...
          () => ({ action: 'drop', args: { bet: autoBetValue(settings.aggression), risk: pick(['low', 'medium', 'medium', 'high']), rows: pick([8, 12, 12, 16]) } }),
        )

        const bet = applyBet(dec.args.bet)
        const risk = ['low', 'medium', 'high'].includes(dec.args.risk as string) ? (dec.args.risk as string) : 'medium'
        const riskBtn = document.querySelector<HTMLElement>(`[data-risk="${risk}"]`)
        if (riskBtn) riskBtn.click()
        const rows = [8, 12, 16].includes(Number(dec.args.rows)) ? Number(dec.args.rows) : 12
        const rowsBtn = document.querySelector<HTMLElement>(`[data-rows="${rows}"]`)
        if (rowsBtn) rowsBtn.click()
        ctx.setStatus(`Dropping a ${risk}-risk ball (${rows} rows), bet ${fmt(bet)}…`)
        await sleep(120)
        $('#dropBtn').click()
  ...
  async function chooseGame(_ctx: { shouldStop: () => boolean; setStatus: (msg: string) => void }, GAMES: string[], excludeId: string | null) {
  ...
  async function switchToGame(gid: string) {
  ...
  function sessionStatusLine() {
    ...
    let line = `${adapters[gid] ? gid + ' · ' : ''}round ${s.rounds} · net ${s.net >= 0 ? '+' : '-'}${money(s.net)} · ${s.tiltEmoji} ${s.tiltShort}`
    if (s.borrowed > 0) line += ` · 💸 ${money(s.borrowed)} borrowed`
  ...
  async function takeBreak(ms: number) {
    const startedAt = Date.now()
    const endAt = startedAt + ms
    logEvent({ kind: 'break', phase: 'start', plannedMs: ms })
    toast('AI on a scheduled break — inference paused.', 'info')
  ...
  async function desperateFinancing(ctx: { shouldStop: () => boolean; setStatus: (msg: string) => void }) {
    ...
    wallet.payout(scen.amount) // inject the borrowed money into the wallet
    ...
    logEvent({ kind: 'loan', label: scen.label, amount: scen.amount, reason, balance: wallet.getBalance() })
    // Legacy passed a 4th "duration" arg here (Casino.toast(msg,type,4600)) —
    // toastStore's toast() is a fixed ~3.6s auto-dismiss (plan §4), so that
    // argument has no equivalent and is dropped (plan §0.3.1).
    toast(`💸 ${scen.label} — took ${money(scen.amount)} to keep playing.`, 'lose')
    onUpdate()
    await pace()
    return true
  }

  async function runLoop(startId?: string) {
    if (running) return
    ...
    if (!settings.agentic && !adapters[fixedId]) {
      ...
      toast('Open a game first, or enable Agentic mode.', 'info')
      return
    }
    running = true
    stopFlag = false
    ...
    try {
      ...
      if (reason) { setStatus(`Stopped: ${reason}.`); toast(`AI stopped — ${reason}.`, 'info') }
    } catch (err) {
      if (stopFlag) {
        ...
        toast('AI stopped: can\'t reach Ollama (check it\'s running + CORS).', 'lose')
      } else {
        console.error('[agent] loop error:', err)
        setStatus('⚠️ ' + (err instanceof Error ? err.message : String(err)))
      }
    } finally {
      running = false
      stopFlag = false
  ...
  function forceNewGame() {
    if (!running) { toast('Start the AI first.', 'info'); return }
    if (!settings.agentic) { toast('Turn on Agentic mode so the AI can roam between games.', 'info'); return }
    pendingSwitch = true
    setStatus('Switching to a new game after this round…')
    toast('AI will move to a new game.', 'info')
  }
  ...
  function stopAndReport() {
    if (running) {
      ...
      void generateReportNow('manual review (session already stopped)')
    }
  }
  async function generateReportNow(trigger?: string) {
    if (!rounds.length) { toast('No session data yet — start the AI first.', 'info'); return }
  ...
  function dominantGame(fromN: number, toN: number) {
  ...
  function analyzeSwings() {
    const start = session ? session.startCredits : wallet.getBalance()
    const pts = [{ balance: start, n: 0 }, ...rounds.map((r) => ({ balance: r.balance, n: r.n }))]
    let minIdx = 0, up = { amount: 0, from: 0, to: 0, game: null as string | null }
    for (let j = 1; j < pts.length; j++) {
      if (pts[j].balance - pts[minIdx].balance > up.amount) up = { amount: pts[j].balance - pts[minIdx].balance, from: pts[minIdx].n, to: pts[j].n, game: null }
      if (pts[j].balance < pts[minIdx].balance) minIdx = j
    }
    let maxIdx = 0, down = { amount: 0, from: 0, to: 0, game: null as string | null }
    for (let j = 1; j < pts.length; j++) {
      if (pts[maxIdx].balance - pts[j].balance > down.amount) down = { amount: pts[maxIdx].balance - pts[j].balance, from: pts[maxIdx].n, to: pts[j].n, game: null }
  ...
  function extractBreaks() {
    const breaks: Array<{ start: number; end: number; durationMs: number }> = []
    let open: number | null = null
    for (const ev of log) {
      if (ev.kind === 'break' && ev.phase === 'start') open = ev.t ?? Date.now()
      else if (ev.kind === 'break' && ev.phase === 'end' && open) { breaks.push({ start: open, end: ev.t ?? Date.now(), durationMs: (ev.t ?? Date.now()) - open }); open = null }
    }
    return breaks
  }

  function sampleRounds<T>(arr: T[], k: number): T[] {
    if (arr.length <= k) return arr
    const step = arr.length / k, out: T[] = []
  ...
  function buildReportPrompt(r: {
    ...
    borrowed: number
    netWithDebt: number
    desperateMeasures: Array<{ label: string }>
    ...
    rounds: AgentRound[]
    breaks: unknown[]
    swings: ReturnType<typeof analyzeSwings>
  }) {
    const s = r.stats
  ...
  async function generateReport(trigger: string) {
    const s = stats()
    const start = session ? session.startCredits : wallet.getBalance()
    const end = wallet.getBalance()
    ...
    try {
      ...
      report.narrative = '(AI narrative unavailable — could not reach Ollama: ' + (e instanceof Error ? e.message : String(e)) +
  ...
  function hasAdapter(id: string) { return !!adapters[id] }
...
// Module-eval-time singleton (matches the legacy IIFE's own construction
// timing) — the app only ever imports this. createRoyalAgent() itself exists
// so a test can build an isolated instance without fighting shared module
// state (plan §3).
export const royalAgent: RoyalAgent = createRoyalAgent()

// DEV-only console/debugging handle (plan §0.3.3) — parity with
// platform/motion/watchdog.ts's window.__royalMotion. Never ships to prod;
// production code reaches the agent through useAgentStore.
if (import.meta.env.DEV) window.__royalAgentDebug = royalAgent
